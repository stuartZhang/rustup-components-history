...
rm -rf ewhine_pkg diagonal-demo7.tar.gz
mkdir ewhine_pkg
# make version.txt
cd dist
...
cd ..
# make tar file
mv dist ewhine_pkg/diagonal-demo7
cp build.rb ewhine_pkg/
chmod +x ./ewhine_pkg/build.rb
tar -zcvf diagonal-demo7.tar.gz ewhine_pkg/
# Copy tar file to Web-Server static directory
DIST_JOB_DIR="$BUILD_DIST/$JOB_NAME";
TAR_FILE_NAME="diagonal-demo7_${VERSION_NAME}[${BRANCH}].tar.gz"
mkdir -p $DIST_JOB_DIR
cp diagonal-demo7.tar.gz $DIST_JOB_DIR/$TAR_FILE_NAME
