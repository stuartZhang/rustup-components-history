...
pcLog.transformRegister((...args) => window.__mxMacroHooks__[process.env.MACRO_LOG_TRANSFORMER](...args) as MxMacroLogHookParams); // 针对`ie 9`环境，请注释此行。否则，所有日志代码会被忽略，因为`ie 9`不支持`console.log.apply(...)`。
...
window.addEventListener('error', event => {
    if (event.target instanceof HTMLImageElement ||
        ...
        pcLog.error(event.target.tagName, '加载资源失败', event.target.src);
    } else {
        const {stack} = event.error || event;
        if (stack) {
            pcLog.error('window.onerror', stack);
        } else {
            pcLog.error('window.onerror', event.message, `[${event.filename}:${event.lineno}:${event.colno}]`);
        }
    }
}, {
    capture: true
...
window.addEventListener('unhandledrejection', event => {
    pcLog.error('Unhandled rejection (promise: ', event.promise, ', reason: ', event.reason, ').');
});
// =========== Version Log ============
...
pcLog.group(title);
pcLog.table(identity);
pcLog.info(JSON.stringify(identity, null, 2));
pcLog.groupEnd(title);
